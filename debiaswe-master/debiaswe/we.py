...
class WordEmbedding:
            ...
            with open(fname, "r", encoding='utf8') as f:
                ...
                for line in f:
                    ...
                    if len(vecs) and vecs[-1].shape!=v.shape:
                        weird_lines+=1
                        print("Got weird line")
                        continue
    #                 v /= np.linalg.norm(v)
                    else:
                        words.append(s[0])
                        vecs.append(v)
        print("Number of weird lines: ",weird_lines)
    ...
    def reindex(self):
        ...
        # self.index = {k: 0 for  k in self.words}
        # for k in self.index:
        #     self.index[k]=self.words.index(k)
        # print("Buradayım")
        # self.words = list(self.index.keys())
        # self.vecs = self.vecs[list(self.index.values())]
